         ...
         while(true) {
            ...
            switch(reader.readLine()){
                case "1":
                    ...
                    List<SimpleData> buff = service.Select(buffObject, authStringEnc);
                ...
                case "2":
                    buffObject = ReadClass(reader, 0);
                    res = service.Insert(buffObject, authStringEnc);
                    break;
                case "3":
                    buffObject = ReadClass(reader, 1);
                    res = service.Update(buffObject, authStringEnc);
                    break;
                case "4":
                    System.out.println("Input id:");
                    res = service.Delete(reader.readLine(), authStringEnc);
                    break;
            }
            if(res.length() != 0) {
